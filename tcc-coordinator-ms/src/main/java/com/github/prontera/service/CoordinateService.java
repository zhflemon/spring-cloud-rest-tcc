...
public class CoordinateService {
	...
	public void confirm(TccRequest request) {
		...
		if (success > 0 && fail > 0) {
			// 出现冲突必须返回并需要人工介入
			throw new PartialConfirmException("all reservation were cancelled or timeout",
					new TccErrorResponse(participantLinks));
		} else if (fail == participantLinks.size()) {
			// 全部timeout
			throw new ReservationExpireException(
					"although we have check the expire time in request body, we got an expiration when confirming actually");
		}
	}
	...
	private void checkExpireInLocal(TccRequest request, List<Participant> participantLinks) {
		...
		if (theClosestToExpire.minusSeconds(LEEWAY).isBefore(OffsetDateTime.now())) {
			// 释放全部资源
			cancel(request);
			throw new ReservationAlmostToExpireException(
					"there are resources be about to expire at " + theClosestToExpire);
		}
	}
	...
	private OffsetDateTime fetchTheRecentlyExpireTime(List<Participant> participantLink) {
		...
		// 计算出过期时间集合
		final List<OffsetDateTime> dateTimeList = participantLink.stream().flatMap(x -> Stream.of(x.getExpireTime()))
				.filter(x -> x.isAfter(OffsetDateTime.now())).sorted().collect(Collectors.toList());
		// 检查是否具有已经过期的事务
